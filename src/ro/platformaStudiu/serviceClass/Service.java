...
import ro.platformaStudiu.serviceClass.Exceptii.InvalidHourException;
...
import static ro.platformaStudiu.admins.AdaugaUtilizatorPage.getRoluri;
...
public class Service {
    ...
    public static Time valideazaOra(String oraText) throws IllegalArgumentException {
        if (!oraText.matches("^([01]?\\d|2[0-3]):[0-5]\\d(:[0-5]\\d)?$")) {
            throw new InvalidHourException("Formatul orei trebuie să fie HH:mm sau HH:mm:ss!");
        }
        LocalTime ora = LocalTime.parse(oraText);
