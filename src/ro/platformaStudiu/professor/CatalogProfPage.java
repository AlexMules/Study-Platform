...
public class CatalogProfPage {
    ...
    private final TableView<StudentProf> tableView;

    public CatalogProfPage(Stage primaryStage, SqlConexiune sql, int user_id, int profesorID) {
        Label headerLabel = new Label("Catalog Studenți:");
        ...
        inapoi.setOnAction(e -> {
            primaryStage.close();
            new ProfessorPage(primaryStage, sql, 2, user_id);
        });
        ...
        primaryStage.setTitle("Catalog Studenți");
        primaryStage.show();
    ...
    private TableView<StudentProf> initializeTableView() {
        ...
        TableColumn<StudentProf, Double> colNotaCurs = getNotaCursColumn(tableView);
        TableColumn<StudentProf, Double> colNotaSeminar = getNotaSeminarColumn(tableView);
        TableColumn<StudentProf, Double> colNotaLaborator = getNotaLaboratorColumn(tableView);
    ...
    private static TableColumn<StudentProf, Double> getNotaLaboratorColumn(TableView<StudentProf> tableView) {
        ...
        return colNotaLaborator;
    }

    private static TableColumn<StudentProf, Double> getNotaSeminarColumn(TableView<StudentProf> tableView) {
        ...
        return colNotaSeminar;
    }

    private static TableColumn<StudentProf, Double> getNotaCursColumn(TableView<StudentProf> tableView) {
    ...
    private void valideazaNota(String nota, String tipNota, String idStudent) throws MarkException {
        try {
            ...
            if (valoareNota < 1 || valoareNota > 10) {
                throw new MarkException("Nota " + tipNota + " pentru studentul cu ID " + idStudent + " trebuie să fie între 1 și 10!");
            }
        } catch (NumberFormatException e) {
            throw new MarkException("Nota " + tipNota + " pentru studentul cu ID " + idStudent + " este invalidă. Introduceți o valoare numerică!");
        }
    }
}
