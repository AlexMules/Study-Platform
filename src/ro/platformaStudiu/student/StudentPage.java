...
public class StudentPage {
    ...
    public StudentPage(Stage primaryStage, SqlConexiune sql, int user_id){
        ...
        Button activitati = new Button(" Vezi activitățile de astăzi");
        Button vizualizareDateButon= new Button("Vizualizare Date");
        Button inscriereCursuriButon= new Button("Înscriere cursuri");
        Button inscriereActivitatiButon= new Button("Înscriere activități");
        Button deautentificareButton= new Button("Deautentificare");
        Button catalog= new Button("Vezi note");
        Button managementCurs = new Button("Renunțare cursuri");
        ...
        vizualizareDateButon.setOnAction(e->{
            primaryStage.close();
            new VizualizareDate(primaryStage, sql, 3, user_id);
        });

        catalog.setOnAction( e->{
            primaryStage.close();
            int student_id=getStudentId(user_id);
            new CatalogPage(primaryStage, sql, user_id,student_id);
        });

        inscriereCursuriButon.setOnAction(e->{
            primaryStage.close();
            int student_id=getStudentId(user_id);
            new InscriereCursPage(primaryStage,sql, user_id, student_id);
        });

        activitati.setOnAction(e ->{
            primaryStage.close();
            int student_id=getStudentId(user_id);
            new TodayActivitiesPage(primaryStage,sql, user_id, student_id);
        });

        managementCurs.setOnAction(e->{
            primaryStage.close();
            int student_id=getStudentId(user_id);
            new RenuntareCurs(primaryStage,sql, student_id, user_id);
        });

        inscriereActivitatiButon.setOnAction(e->{
            primaryStage.close();
            int student_id=getStudentId(user_id);
            new ActivitatiSelectStudentPage(primaryStage,sql, user_id, student_id);
        });

        grupuriDeStudiu.setOnAction(e->{
            primaryStage.close();
            int student_id=getStudentId(user_id);
            new GrupStudiuPage(primaryStage,sql, user_id, student_id);
        });
