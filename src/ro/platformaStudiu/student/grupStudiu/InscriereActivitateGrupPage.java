...
public class InscriereActivitateGrupPage {
    ...
    public InscriereActivitateGrupPage(Stage primaryStage, SqlConexiune sql, int userID, int studentId, GrupStudiu grup) {
        ...
        Button inscriereButton = getButton(studentId, activitatiComboBox, statusLabel);
    ...
    private Button getButton(int studentId, ComboBox<ActivitateGrup> activitatiComboBox, Label statusLabel) {
    ...
    private void inscriereStudentLaActivitate(int studentId, ActivitateGrup activitate, Label statusLabel) {
        ...
        try (PreparedStatement stmt = SqlConexiune.con.prepareStatement(query)) {
            ...
            e.printStackTrace();
            statusLabel.setText("Eroare! Ești deja înscris la această activitate!");
            statusLabel.setTextFill(Color.RED);
        }
    }
