...
public class ManagementGrupuri {
    ...
    public ManagementGrupuri(Stage primaryStage, SqlConexiune sql, int userID, int studentID){
        ...
        veziStudenti.setOnAction(e -> {
            ...
            if (grup != null) {
                primaryStage.close();
                new TableStudentiPage(primaryStage, sql, userID, studentID, grup);
            }
        });
        ...
        mesaj.setOnAction(e -> {
            ...
            if (grup != null) {
                primaryStage.close();
                new TrimiteMesajPage(primaryStage, sql, userID, studentID, grup);
            }
        });
        ...
        chat.setOnAction(e->{
            ...
            if (grup != null) {
                primaryStage.close();
                new ChatPage(primaryStage, sql, userID, studentID, grup);
            }
        });
        Button adaugaActivitate=new Button("Adaugă o activitate la grup");
        adaugaActivitate.setFont(new Font("Times New Roman", 14));
        adaugaActivitate.setOnAction(e->{
            GrupStudiu grup = getGrupNullEx(statusLabel, grupuriComboBox);
            if (grup != null) {
                primaryStage.close();
                new ActivitateGrupPage(primaryStage, sql, userID, studentID, grup);
            }
        });
        Button inscriereActivitate = new Button("Înscriere activitate grup");
        inscriereActivitate.setFont(new Font("Times New Roman", 14));
        inscriereActivitate.setOnAction(e->{
            GrupStudiu grup = getGrupNullEx(statusLabel, grupuriComboBox);
            if (grup != null) {
                primaryStage.close();
                new InscriereActivitateGrupPage(primaryStage, sql, userID, studentID, grup);
            }
        });
